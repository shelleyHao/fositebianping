...
//        }
    ...
    //    分类导航列表
    public function nav_cat_tree(){
        $GoodsLogic = new GoodsLogic();
        $cat_list = $GoodsLogic->goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 1);
  ...
  * */
    ...
    //    查找某一/所有分类的商品
    public function productList($cat_id = 0,$page=3){
        $cat_id = input('get.cat_id');
//       dump(input('get.cat_id'));
        if($cat_id > 0){
            $list =  Db::name('goods')->where('cat_id',$cat_id)->paginate($page);
        }else{
            $list = Db::name('goods')->paginate($page);
        }
        return $list;
    }
}
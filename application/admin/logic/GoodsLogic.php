...
class GoodsLogic extends Model
{
    ...
    public function afterSave($goods_id)
    {
        ...
        $max_price = Db::name('goods')->where("goods_id = $goods_id")->value('max_price');
        $min_price = Db::name('goods')->where("goods_id = $goods_id")->value('min_price');
        if ($goods_item) {
            $keyArr = '';//规格key数组
            foreach ($goods_item as $k => $v) {
                ...
                M('cart')->where("goods_id = $goods_id and spec_key = '$k'")->save(array(
                    'max_price' => $max_price, //$v['max_price'], //市场价

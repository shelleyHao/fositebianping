    ...
    public  function addEditNav(){
        ...
        if(!empty($cat_list))
        {
            foreach ($cat_list AS $key => $value)
            {
                ...
                $select_option[$select_val] = $value['name'];
            }
        }
        $system_nav = array();

        $article_cat_list = M('article_cat')->where('show_in_nav',0)->select();
        if(!empty($article_cat_list)){
            foreach ($article_cat_list AS $key => $value){
                $art_select_val = U("/Home/Article/articleList",array('id'=>$key));
                $art_select_option[$art_select_val] = $value['cat_name'];
            }
        }

        $news_cat_list = M('news_cat')->where('show_in_nav',0)->select();
        if(!empty($news_cat_list)){
            foreach ($news_cat_list AS $key => $value){
                $new_select_val = U("/Home/News/newsList",array('id'=>$key));
                $new_select_option[$new_select_val] = $value['cat_name'];
            }
        }

        $system_nav = array_merge($system_nav,$new_select_option);
        $system_nav = array_merge($system_nav,$art_select_option);
